...
import { submittedArticlesDBConnect } from "@/utils/connections";

export const dynamic = "force-dynamic";

export async function GET(req: NextRequest) {
	// Connect to the database
	const db = await submittedArticlesDBConnect();
	console.log("Connected to db");
	// Get all articles
	const articles = await db.ArticleSubmission.find({}).exec();
...
export async function POST(req: NextRequest) {
	...
	// no empty fields
	if (!author || !email || !buID || !major || !gradYear || !title || !subsection || !articleLink || !approval) {
	...
	const newSubmission = new db.ArticleSubmission({
		author,
		email,
		buID,
		major,
